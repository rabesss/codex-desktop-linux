...
4. Bundled resources are staged: Browser Use, Chrome native messaging, Linux
   Computer Use, and enabled maintained feature resources.
...
- Preserve the direct official OpenAI/Codex route and keep
  [`rabesss/codex-shim`](https://github.com/rabesss/codex-shim) as an opt-in
  custom-provider path only. This repository owns Desktop picker/session/browser
  behavior; the shim owns model discovery and protocol translation.
- Do not reintroduce third-party Computer Use plugin staging. Hyprland/X11
  support belongs in the owned `computer-use-linux` backend or in reviewed
  source committed to this repo.
- Agent Workspaces, Read Aloud, Read Aloud MCP, Conversation Mode, AppShots,
  Remote Control UI, Remote Mobile Control, Zed opener, Copilot reasoning
  defaults, and the example feature were removed from this fork. Do not stage or
  re-add those surfaces unless there is a new explicit product decision,
  reviewed implementation, tests, and update-builder coverage.
- Keep `profiles/workstation/features.json` as the workstation feature profile.
  Do not commit `linux-features/features.json`.
- Preserve Brave Origin Nightly targeting and XWayland launch behavior when
  producing the maintained workstation profile. Browser-control changes are on
  `main`; do not recreate the removed `plugins/browser-control-linux` branch.
- Run `scripts/workstation/verify-policy.sh` after changes to plugins, features,
  updater behavior, packaging, or docs.
...
- `scripts/bootstrap-wizard.sh`
  ...
  feature config, validate feature relationships, and install native packages.
- `Cargo.toml`
  Workspace root. Members currently are `computer-use-linux` and `updater`.
...
- `bundled-plugins.sh`
  ...
  helper binaries, and marketplace metadata. Maintained optional features can
  stage additional reviewed bundled-style resources.
...
### Computer Use, Browser, And Archived Voice Code
...
- `install-user-local.sh`
  Installs under `~/.local/opt/codex-desktop-control`, creates wrappers under
  `~/.local/bin`, and installs a user desktop entry.
- `files/.local/lib/codex-desktop-control/common.sh`
