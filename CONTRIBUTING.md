...
Changes to custom-model behavior may span the companion
[`rabesss/codex-shim`](https://github.com/rabesss/codex-shim) repository.
Keep ownership explicit: this repo owns Desktop picker/session/browser behavior;
the shim owns model discovery and wire-protocol translation. When a change
crosses that boundary, link the companion commit and run focused tests in both
repositories.
