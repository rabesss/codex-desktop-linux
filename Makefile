...
CUSTOM_MODELS_FEATURES_CONFIG ?= $(CURDIR)/profiles/custom-models/features.json
MAX_BUILD_THREADS ?= 0
...
help:
	...
	@printf '  %-18s %s\n' "make install-guided" "Guided feature selection plus dependencies, build, package, and install"
	...
	@printf '  %-18s %s\n' "make install-custom-models" "Fresh-build and install with the public custom-model profile"
	@printf '  %-18s %s\n' "make package-custom-models" "Fresh-build and package with the public custom-model profile"
	...
	@printf '  %-18s %s\n' "CUSTOM_MODELS_FEATURES_CONFIG=..." "Override the feature manifest used by custom-model targets"
...
install-guided:
	@echo "[make] Running guided native setup and install"
...
install-custom-models:
	CODEX_LINUX_FEATURES_CONFIG="$(CUSTOM_MODELS_FEATURES_CONFIG)" $(MAKE) install-native

package-custom-models:
	CODEX_LINUX_FEATURES_CONFIG="$(CUSTOM_MODELS_FEATURES_CONFIG)" $(MAKE) build-app-fresh
	CODEX_LINUX_FEATURES_CONFIG="$(CUSTOM_MODELS_FEATURES_CONFIG)" $(MAKE) package
