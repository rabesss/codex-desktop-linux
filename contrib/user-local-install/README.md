...
This folder packages a user-local install layout for `codex-desktop-control`.

It adds:

- a stable install root under `~/.local/opt/codex-desktop-control`
- self-contained maintenance scripts under `~/.local/opt/codex-desktop-control/bin`
...
- `~/.local/opt/codex-desktop-control/bin/`
- `~/.local/opt/codex-desktop-control/lib/codex-desktop-control/`
- `~/.local/bin/` wrappers
...
- `~/.local/opt/codex-desktop-control/bin/`
- `~/.local/opt/codex-desktop-control/lib/codex-desktop-control/`
- `~/.local/bin/` wrappers that exec into `~/.local/opt/codex-desktop-control/bin/`
- `~/.local/share/applications/`
- `~/.config/systemd/user/`

The preferred git checkout location is:

- `~/workspace/codex-desktop-control`

The installed maintenance scripts record the repo path in user state and use that checkout for `git pull`, while rebuilding runtime assets into `~/.local/opt/codex-desktop-control` via `CODEX_INSTALL_ROOT` / `CODEX_INSTALL_DIR`.
...
1. copies standalone helper scripts into `~/.local/opt/codex-desktop-control`
...
- The X11/XWayland preference is stored in `~/.config/codex-desktop-control/user-local.env` and is preserved across updater refreshes.
