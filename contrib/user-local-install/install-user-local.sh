...
OPT_ROOT="${HOME}/.local/opt/codex-desktop-control"
OPT_BIN_DIR="${OPT_ROOT}/bin"
OPT_LIB_DIR="${OPT_ROOT}/lib/codex-desktop-control"
XDG_DATA_HOME="${XDG_DATA_HOME:-${HOME}/.local/share}"
XDG_CONFIG_HOME="${XDG_CONFIG_HOME:-${HOME}/.config}"
DATA_DIR="${XDG_DATA_HOME}/codex-desktop-control"
CONFIG_DIR="${XDG_CONFIG_HOME}/codex-desktop-control"
USER_LOCAL_ENV_FILE="${CONFIG_DIR}/user-local.env"
MANAGED_REPO_DIR="${DATA_DIR}/managed-repo"
STATE_DIR="${XDG_STATE_HOME:-${HOME}/.local/state}/codex-desktop-control"
FROM_UPDATE=0
ENABLE_TIMER=0
...
install_manager_files() {
    ...
    copy_file "${FILES_DIR}/.local/lib/codex-desktop-control/common.sh" "${OPT_LIB_DIR}/common.sh"
...
#!/usr/bin/env bash
set -euo pipefail
exec "${HOME}/.local/opt/codex-desktop-control/bin/codex-desktop" "$@"
EOF
    cat > "${HOME}/.local/bin/codex-desktop-check-update" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec "${HOME}/.local/opt/codex-desktop-control/bin/codex-desktop-check-update" "$@"
EOF
    cat > "${HOME}/.local/bin/codex-desktop-update" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec "${HOME}/.local/opt/codex-desktop-control/bin/codex-desktop-update" "$@"
EOF
    cat > "${HOME}/.local/bin/codex-desktop-version" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec "${HOME}/.local/opt/codex-desktop-control/bin/codex-desktop-version" "$@"
EOF
