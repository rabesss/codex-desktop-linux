...
| Custom rows are missing from the model picker | Confirm `custom-model-catalog` was enabled at build time and `curl http://127.0.0.1:8765/api/models` returns a catalog |
...
| Custom model context footer or compaction threshold is wrong | Regenerate the custom catalog, or update `codex-shim` and run `codex-shim desktop write-models` if using shim, then restart Desktop so the feature CLI wrapper rebuilds the merged app-server catalog. Check the custom catalog, `curl http://127.0.0.1:8765/api/models` for shim rows, and `$XDG_STATE_HOME/codex-desktop/custom-model-catalog/merged-model-catalog.json` for `context_window` and `auto_compact_token_limit`. |
