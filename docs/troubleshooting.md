...
| The in-app `sha` chip looks stale or does not match the DMG hash | The chip shows the installed Linux wrapper/source commit, not the upstream DMG SHA256. Compare `source.commit` and `upstreamDmg.sha256` in `/opt/codex-desktop/resources/codex-linux-build-info.json`, then compare them with `installed_wrapper_commit` and `dmg_sha256` from `codex-update-manager status --json`. |
