...
const MODEL_QUERY_SHIM_HELPER_SOURCE = [
  ...
  "function codexLinuxCustomModelToRow(e,t){let n=codexLinuxCustomModelString(e.slug);if(!n)return null;let r=codexLinuxCustomModelArray(e.input_modalities??e.inputModalities);r.length||(r=[`text`]);let i=codexLinuxCustomModelString(e.model_provider??e.modelProvider)??`codex_shim`,a=codexLinuxCustomModelString(e.provider_display_name??e.providerDisplayName??e.provider_name??e.providerName)??codexLinuxCustomModelString(t?.get(i)?.name)??codexLinuxCustomModelString(e.provider??e.owned_by??e.ownedBy)??i,o=codexLinuxCustomModelString(e.display_name??e.displayName??e.name??n)??n,s=codexLinuxCustomModelString(e.description)??`${o} via ${a}.`,c=codexLinuxCustomModelString(e.upstream_model_id??e.upstreamModelId??e.model??n)??n,l=codexLinuxCustomModelString(e.source)??(i===`codex_shim`?`CLIProxyAPI/local adapter`:`custom catalog`),u=codexLinuxCustomModelString(e.context_window??e.max_context_window??e.contextWindow??e.maxContextWindow),d=codexLinuxCustomModelString(e.model_catalog_json??e.modelCatalogJson),f=e.auto_compact_token_limit??e.autoCompactTokenLimit??null,p=e.truncation_policy??e.truncationPolicy??null;return{model:n,displayName:o,description:s,hidden:!1,isDefault:!1,modelProvider:i,model_provider:i,owned_by:codexLinuxCustomModelString(e.owned_by??e.ownedBy)??i,provider:i,providerDisplayName:a,upstreamModelId:c,source:l,contextWindow:u,modelCatalogJson:d,autoCompactTokenLimit:f,truncationPolicy:p,inputModalities:r,supportedReasoningEfforts:codexLinuxCustomModelReasoning(e.supported_reasoning_efforts??e.supportedReasoningEfforts),defaultReasoningEffort:codexLinuxCustomModelString(e.default_reasoning_effort??e.defaultReasoningEffort)??`medium`,supportsTools:e.supports_tools===!0||e.supportsTools===!0,supportsReasoning:e.supports_reasoning===!0||e.supportsReasoning===!0,supportsStreaming:e.supports_streaming!==!1&&e.supportsStreaming!==!1,supportsImageInputs:r.includes(`image`),supportsImageDetailOriginal:e.supports_image_detail_original===!0||e.supportsImageDetailOriginal===!0}}",
  "async function codexLinuxCustomModelMergeListModels(e){try{let t=await fetch(`http://127.0.0.1:8765/api/models`,{cache:`no-store`});if(!t.ok)return e;let n=await t.json(),r=codexLinuxCustomModelProviderConfigs(n),i=codexLinuxCustomModelCatalogRows(n).map(e=>codexLinuxCustomModelToRow(e,r)).filter(Boolean),a=e&&typeof e==`object`?e:{data:[]},o=codexLinuxCustomModelArray(a.data),s=new Set(o.map(e=>e?.model).filter(Boolean)),c=new Set,l=i.filter(e=>{let t=`${e.providerDisplayName}\\0${e.displayName}`;return!s.has(e.model)&&!c.has(t)&&(c.add(t),!0)}),u=e=>typeof e==`string`?e.trim().toLowerCase():``;globalThis.__codexLinuxCustomModelSlugs=new Set(l.map(e=>u(e.model))),globalThis.__codexLinuxCustomModelCatalogPaths=new Map(l.flatMap(e=>e.modelCatalogJson==null?[]:[[u(e.model),e.modelCatalogJson]])),globalThis.__codexLinuxCustomModelRuntimeConfig=new Map(l.flatMap(e=>{let t=codexLinuxCustomModelRuntimeConfig(e);return Object.keys(t).length===0?[]:[[u(e.model),t]]})),globalThis.__codexLinuxCustomModelProviders=new Map(l.map(e=>[u(e.model),e.modelProvider])),globalThis.__codexLinuxCustomModelProviderConfigs=new Map([...r].filter(([e])=>l.some(t=>t.modelProvider===e)));return{...a,data:[...o,...l]}}catch{return e}}",
].join("");
