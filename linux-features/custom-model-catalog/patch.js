...
const MODEL_QUERY_SHIM_HELPER_SOURCE = [
  ...
  "async function codexLinuxCustomModelMergeListModels(e){try{let t=await codexLinuxCustomModelFetchCatalogs(),n=codexLinuxCustomModelMergeProviderConfigs(t),r=t.flatMap(e=>codexLinuxCustomModelCatalogRows(e)).map(e=>codexLinuxCustomModelToRow(e,n)).filter(Boolean),i=e&&typeof e==`object`?e:{data:[]},a=codexLinuxCustomModelArray(i.data),o=e=>typeof e==`string`?e.trim().toLowerCase():``,s=new Set(a.flatMap(e=>[o(e?.model),o(e?.id)]).filter(Boolean)),c=new Map,l=new Map(a.flatMap(e=>{let t=o(e?.model??e?.id);return t?[[t,e]]:[]})),u=new Set,d=[];for(let e of r){let t=o(e.model),n=`${e.providerDisplayName}\\0${e.displayName}`;if(!t)continue;let r=l.get(t);if(s.has(t)&&!codexLinuxCustomModelExistingRowMatches(r,e))continue;d.push(e);if(u.has(n))continue;u.add(n);c.has(t)||c.set(t,e)}let f=new Set,p=[...c.values()].filter(e=>{let t=`${e.providerDisplayName}\\0${e.displayName}`;return!s.has(o(e.model))&&!f.has(t)&&(f.add(t),!0)}),m=d,g=e=>{let t=o(e.model),n=o(e.upstreamModelId);return n&&n!==t&&!s.has(n)?[t,n]:[t]};globalThis.__codexLinuxCustomModelSlugs=new Set(m.flatMap(g)),globalThis.__codexLinuxCustomModelCatalogPaths=new Map(m.flatMap(e=>e.modelCatalogJson==null?[]:g(e).map(t=>[t,e.modelCatalogJson]))),globalThis.__codexLinuxCustomModelRuntimeConfig=new Map(m.flatMap(e=>{let t=codexLinuxCustomModelRuntimeConfig(e);return Object.keys(t).length===0?[]:g(e).map(n=>[n,t])})),globalThis.__codexLinuxCustomModelProviders=new Map(m.flatMap(e=>g(e).map(t=>[t,e.modelProvider]))),globalThis.__codexLinuxCustomModelWireModels=new Map(m.flatMap(e=>{let t=codexLinuxCustomModelString(e.upstreamModelId);return t?g(e).map(e=>[e,t]):[]})),globalThis.__codexLinuxCustomModelToolSupport=new Map(m.flatMap(e=>g(e).map(t=>[t,e.supportsTools===!0]))),globalThis.__codexLinuxCustomModelProviderConfigs=new Map([...n].filter(([e])=>m.some(t=>t.modelProvider===e)));return{...i,data:[...a,...p]}}catch{return e}}",
].join("");
