#!/usr/bin/env bash
# Guided, conservative setup helper for native Codex Desktop Control builds.
set -Eeuo pipefail
...
Environment:
  ...
  CODEX_BOOTSTRAP_DRY_RUN=1            preview install actions without changing them
...
window_backend_hint() {
    ...
    if [[ "$desktop" == *hyprland* ]]; then
        ...
        printf 'Sway -> swaymsg IPC backend'
...
print_system_summary() {
    ...
    info "Codex Desktop Control guided setup"
...
if features:
    ...
    for index, (feature_id, feature) in enumerate(features.items(), start=1):
        ...
        print(f"[setup]   {index}. [{state}] {feature_id}{local} - {feature['title']}")
else:
    print("[setup] Available Linux features: none found")

if apply_changes and (enable or disable):
    print("[setup] Feature changes apply after rebuilding and reinstalling Codex Desktop Control.")
PY
    then
        SETUP_ERROR_REPORTED=1
...
prompt_for_feature_changes_gui() {
    ...
    if command -v zenity >/dev/null 2>&1; then
        ...
        selected="$(zenity --list --checklist \
            --title="Codex Desktop Control features" \
...
main() {
    ...
    if dry_run_enabled; then
        ...
        info "Setup completed without changing services, groups, key files, model files, or plugin caches."
    fi
