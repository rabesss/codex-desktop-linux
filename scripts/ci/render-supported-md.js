...
const WORKFLOWS = [
  {
    label: "CI",
    file: "ci.yml",
    validates: "Rust, smoke tests, Debian/RPM/pacman fixture packages, Nix flake outputs",
  },
  {
    label: "Upstream Build App",
    file: "upstream-build-app.yml",
    validates: "Live upstream DMG patch/build validation and metadata-only candidate reports",
  },
  {
  ...
  },
  {
    ...
    file: "cachix.yml",
    validates: "Optional Nix cache population when repository secrets are configured",
  },
];
...
function renderNixPinsSection(nixPins) {
  ...
  return [
    "## Nix Flake Pins",
    "",
    "Nix packaging is a separate validation surface from the default updater approval lock. The flake can be refreshed to keep Nix builds green while the default end-user updater still follows the approved upstream app pin above.",
    "",
    "| Field | Value |",
    "| --- | --- |",
...
CI checks this file on every push and pull request. The badges below are live GitHub workflow status links for \`${repository}\`.
...
| Nix | Checked flake evaluation and package outputs from the Nix flake pins listed below | Treating the Nix pin as automatic approval for the default updater channel |
...
| \`CI / Nix Package Builds\` | Nix flake evaluation and package outputs match the committed Nix app, Electron, native-module, and bundled CLI pins. |
| \`Upstream Build App\` | GitHub Actions can download the live official DMG, patch/build it, validate required patch points, and upload metadata-only candidate evidence. |
...
The default supported upstream app version is the approved pin above. A newer upstream DMG is a candidate until CI validation, local dogfood, and a reviewed promotion update the lock.
