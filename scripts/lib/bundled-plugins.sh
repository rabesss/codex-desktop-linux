...
stage_chrome_plugin_from_upstream() {
    ...
    patch_browser_use_node_repl_capability_guard "$target_plugin/scripts/browser-client.mjs"
...
updated = re.sub(
    r'return console\.warn\("codexLinuxSiteStatusAllowlistFallback",[^)]*\),!1',
    'return void "codexLinuxSiteStatusAllowlistFallback",!1',
    source,
)
if updated != source:
...
replacement = (
    ...
    f'String({error}?.message??{error}).toLowerCase().includes("allowlist"))return void '
    f'"codexLinuxSiteStatusAllowlistFallback",!1;throw {error}}}'
...
patch_browser_use_documentation_notes() {
    local plugin="$1"
...
def insert_after_frontmatter_once(path: Path, marker: str, block: str) -> None:
    if not path.exists():
        return
    text = path.read_text(encoding="utf-8")
    if marker in text:
    ...
    if text.startswith("---\n"):
        ...
        if closing != -1:
            ...
            return
    prepend_once(path, marker, block)
...
api_notes = """## Linux Desktop API Notes
- `tab.url` and `tab.title` are async methods. Use `await tab.url()` and `await tab.title()`; do not read `tab.url` or `tab.title` as properties.
- Navigation waits live on `tab.playwright.waitForLoadState({ state: "load" })`, `tab.playwright.waitForLoadState({ state: "domcontentloaded" })`, `tab.playwright.waitForURL(...)`, or `tab.playwright.expectNavigation(...)`. There is no `tab.waitForLoadState` method.
- `tab.dom_cua.get_visible_dom()` returns a newline-separated string of visible interactable nodes, not an array or JSON object. Parse the `node_id=...` attributes from that string before calling DOM CUA actions.
- In the in-app Browser backend, file uploads are not available through `locator.setInputFiles(...)`; use the Chrome backend file chooser flow when upload automation is required.
"""

playwright_notes = """### Linux Desktop Evaluate And Wait Notes
- `tab.playwright.evaluate(...)` accepts either an actual JavaScript function object, such as `await tab.playwright.evaluate(() => document.title)`, or a JavaScript expression string, such as `await tab.playwright.evaluate("document.title")`.
- Do not pass a function-shaped string such as `"() => document.title"` unless you immediately invoke it as an expression, for example `"(() => document.title)()"`. A bare function-shaped string evaluates to a function value, not its return value.
- String object literals must be parenthesized, for example `"({ a: 1 })"`.
- The page scope is read-only; use `evaluate(...)` for reads and locator/CUA APIs for interactions.
- For page settling, use `tab.playwright.waitForLoadState({ state: "load" })`, `tab.playwright.waitForLoadState({ state: "domcontentloaded" })`, `tab.playwright.waitForURL(...)`, or `tab.playwright.expectNavigation(...)`. Do not use `networkidle` in this backend.
"""

skill_notes = """## Linux Desktop Runtime Notes
- `tab.url` and `tab.title` are async methods: use `await tab.url()` and `await tab.title()`.
- `tab.playwright.evaluate(...)` runs expression strings as expressions. If you write a function-shaped string, invoke it as an IIFE; otherwise pass an actual JavaScript function object.
- `tab.dom_cua.get_visible_dom()` returns a string of visible node lines, not JSON.
- Use `tab.playwright.waitForLoadState(...)`, `waitForURL(...)`, or `expectNavigation(...)`; there is no `tab.waitForLoadState` helper.
"""

prepend_once(root / "docs" / "api.md", "## Linux Desktop API Notes", api_notes)
prepend_once(root / "docs" / "playwright.md", "### Linux Desktop Evaluate And Wait Notes", playwright_notes)
insert_after_frontmatter_once(
    root / "skills" / "control-in-app-browser" / "SKILL.md",
...
replace_once(
    ...
    "get_visible_dom(): Promise<string>; // Return newline-separated visible interactable DOM lines with node_id attributes.",
)
replace_once(
    root / "docs" / "playwright.md",
    "Do not assume `locator(...).selectOption(...)` exists in this environment.",
    "`locator(...).selectOption(...)` is available only where the API reference documents it and the target is a real `<select>` element.",
)
PY
}
...
patch_browser_use_node_repl_capability_guard() {
    local client="$1"

    if grep -q "codexLinuxNodeReplCapabilityGuard" "$client"; then
        return 0
    fi
...
if match is None:
    if (
        "Browser security unavailable outside node repl" in source
        and "globalThis.nodeRepl" in source
        and ".config" in source
    ):
        print(
            "ERROR: Browser Use still has an unrecognized nodeRepl config guard",
            file=sys.stderr,
        )
        raise SystemExit(1)
    print(
        "INFO: Browser Use nodeRepl capability guard is not required by this bundle",
        file=sys.stderr,
    )
    raise SystemExit(0)
...
value = match.group("value")
replacement = (
    f'function {helper}(){{let {value}=globalThis.nodeRepl;'
    f'return {value}==null||typeof {value}.createElicitation!="function"||'
    f'typeof {value}.fetch!="function"?void 0:{value}'
    f'/*codexLinuxNodeReplCapabilityGuard*/}}'
)
path.write_text(source[:match.start()] + replacement + source[match.end():], encoding="utf-8")
PY
}
...
stage_browser_plugin_from_upstream() {
    ...
    patch_browser_use_node_repl_capability_guard "$target_client"
