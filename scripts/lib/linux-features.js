...
const LEGACY_FEATURE_INSTALL_CLEANUP = new Map();
const LEGACY_BUNDLED_PLUGIN_MARKETPLACE_CLEANUP = new Map();
...
function removeLegacyBundledPluginMarketplaceEntry(installDir, pluginName) {
  ...
  try {
    ...
    console.warn(`WARN: Could not clean bundled plugin marketplace at ${marketplacePath}: ${error.message}`);
    return;
  }
  ...
  console.error(`Removed disabled Linux feature marketplace entry: ${pluginName}`);
}

function removeDisabledLegacyFeatureArtifacts(installDir, options = {}) {
  const enabled = new Set(enabledLinuxFeatureIds(options));
  for (const [featureId, relativePaths] of LEGACY_FEATURE_INSTALL_CLEANUP.entries()) {
    ...
    for (const relativePath of relativePaths) {
      const { normalized, resolved } = resolveInstallRelativePath(
        installDir,
        relativePath,
        `Disabled Linux feature '${featureId}' cleanup target`,
      );
      if (!fs.existsSync(resolved)) {
        continue;
      }
      fs.rmSync(resolved, { recursive: true, force: true });
      console.error(`Removed disabled Linux feature artifact: ${featureId} -> ${normalized}`);
    }
  }
  for (const [featureId, pluginNames] of LEGACY_BUNDLED_PLUGIN_MARKETPLACE_CLEANUP.entries()) {
    if (enabled.has(featureId)) {
      continue;
    }
    for (const pluginName of pluginNames) {
      removeLegacyBundledPluginMarketplaceEntry(installDir, pluginName);
    }
  }
}
...
function stageEnabledLinuxFeatureInstall(appDir, options = {}) {
  ...
  removeDisabledLegacyFeatureArtifacts(installDir, options);
...
module.exports = {
  ...
  linuxFeaturesRoot,
  removeDisabledLegacyFeatureArtifacts,
