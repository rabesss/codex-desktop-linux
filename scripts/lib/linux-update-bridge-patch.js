...
function migrateLinuxUpdaterBridgeSource(source) {
  ...
  patchedSource = replaceAfter(
    patchedSource,
    bootstrapNeedle,
    "manager:{getIsUpdateReady:()=>s&&t,getUpdateLifecycleState:()=>s?n:`idle`,getInstallProgressPercent:()=>r,",
    "manager:{hasUpdater:()=>s,getUnavailableReason:()=>s?null:`codex-update-manager unavailable`,getIsUpdateReady:()=>s&&t,getUpdateLifecycleState:()=>s?n:`idle`,getInstallProgressPercent:()=>r,getDownloadProgressPercent:()=>null,getRelaunchNotice:()=>null,showRelaunchNoticeForDebug:()=>{},",
  );
  patchedSource = replaceAfter(
    patchedSource,
    bootstrapNeedle,
...
function applyCurrentBootstrapUpdaterBridgePatch(currentSource) {
  ...
  const appUpdateBroadcasterVar = bridgeHandlersSearchSource.match(
    /([A-Za-z_$][\w$]*)\.broadcastAppUpdateState\(\)/,
  )?.[1] ?? null;
  if (messageDispatcherVar == null && appUpdateBroadcasterVar == null) {
    console.warn("WARN: Could not identify current updater window message dispatcher - skipping Linux updater bridge patch");
    return currentSource;
  }
  const packageUpdateBridgeNotifier = appUpdateBroadcasterVar != null
    ? `broadcast:()=>${appUpdateBroadcasterVar}.broadcastAppUpdateState()`
...
function applyLinuxAppUpdaterMenuPatch(currentSource) {
  let patchedSource = currentSource.replace(
    /([A-Za-z_$][\w$]*)\.setAutomaticBackgroundDownloadsEnabled\(([^()]*)\)/gu,
    (match, receiver, argument, offset) => {
      const guard = `typeof ${receiver}.setAutomaticBackgroundDownloadsEnabled===\`function\`&&`;
      return currentSource.slice(Math.max(0, offset - guard.length), offset) === guard
        ? match
        : `${guard}${match}`;
    },
  );
  if (/[A-Za-z_$][\w$]*=[A-Za-z_$][\w$]*\.[A-Za-z_$][\w$]*\.shouldIncludeSparkle\([A-Za-z_$][\w$]*,process\.platform,process\.env\)\|\|process\.platform===`linux`/.test(patchedSource)) {
    return patchedSource;
  }
  const menuRegex =
    /([A-Za-z_$][\w$]*)=([A-Za-z_$][\w$]*)\.([A-Za-z_$][\w$]*)\.shouldIncludeSparkle\(([A-Za-z_$][\w$]*),process\.platform,process\.env\)/;
  if (!menuRegex.test(patchedSource)) {
    if (patchedSource.includes("enableSparkle") && patchedSource.includes("shouldIncludeSparkle")) {
      console.warn("WARN: Could not find update menu feature gate - skipping Linux update menu patch");
    }
    return patchedSource;
  }
  return patchedSource.replace(menuRegex, "$1=$2.$3.shouldIncludeSparkle($4,process.platform,process.env)||process.platform===`linux`");
}
