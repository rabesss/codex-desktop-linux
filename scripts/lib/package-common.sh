...
update_builder_feature_is_enabled() {
    local feature_id="$1"
    local helper="$REPO_DIR/scripts/lib/linux-features.js"
    local node_bin
    ...
    node_bin="$(package_node_binary)"
    "$node_bin" - "$helper" "$feature_id" <<'NODE'
const helperPath = process.argv[2];
const featureId = process.argv[3];
const { enabledLinuxFeatureIds } = require(helperPath);
process.exit(enabledLinuxFeatureIds().includes(featureId) ? 0 : 1);
NODE
}
...
render_desktop_doctor() {
    local target="$1"
    local package_name

    package_name="$(sed_escape_replacement "$PACKAGE_NAME")"
    sed -e "s/__PACKAGE_NAME__/$package_name/g" \
        "$REPO_DIR/packaging/linux/codex-desktop-doctor.py" >"$target"
    chmod 0755 "$target"
}
...
stage_common_package_files() {
    ...
    local polkit_policy="$REPO_DIR/packaging/linux/com.github.codex-desktop-control.update.policy"
    ...
    render_desktop_doctor "$root/usr/bin/$PACKAGE_NAME-doctor"
    ...
    if package_with_updater_enabled; then
        ...
        cp "$polkit_policy" "$root/usr/share/polkit-1/actions/com.github.codex-desktop-control.update.policy"
        chmod 0644 "$root/usr/share/polkit-1/actions/com.github.codex-desktop-control.update.policy"
    else
        render_no_updater_transition_cleanup_helper \
...
stage_update_builder_bundle() {
    ...
    if [ -d "$APP_DIR/resources/plugins/openai-bundled" ]; then
        mkdir -p "$update_builder_root/resources/plugins"
        cp -a "$APP_DIR/resources/plugins/openai-bundled" \
            "$update_builder_root/resources/plugins/openai-bundled"
    fi
    ...
    cp "$REPO_DIR/scripts/lib/codex-cli-runtime.sh" "$update_builder_root/scripts/lib/codex-cli-runtime.sh"
    ...
    cp "$REPO_DIR/packaging/linux/codex-desktop-doctor.py" \
    ...
    cp "$REPO_DIR/packaging/linux/com.github.codex-desktop-control.update.policy" \
        "$update_builder_root/packaging/linux/com.github.codex-desktop-control.update.policy"
