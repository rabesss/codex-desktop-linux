...
function buildLinuxBuildInfoHelpers(electronVar, fsVar, pathVar) {
  return `function codexLinuxBuildInfoPaths(){let e=[];try{e.push((0,${pathVar}.join)(process.resourcesPath,\`codex-linux-build-info.json\`)),e.push((0,${pathVar}.join)(process.resourcesPath,\`..\`,\`.codex-linux\`,\`build-info.json\`))}catch{}return e}function codexLinuxReadBuildInfo(){for(let e of codexLinuxBuildInfoPaths())try{if(${fsVar}.existsSync(e)){let t=JSON.parse(${fsVar}.readFileSync(e,\`utf8\`));return t&&typeof t===\`object\`&&!Array.isArray(t)?t:null}}catch{}return null}function codexLinuxBuildInfoValue(e,t=\`unknown\`){return typeof e===\`string\`&&e.trim().length>0?e:Array.isArray(e)&&e.length>0?e.join(\`, \`):e==null?t:String(e)}function codexLinuxBuildInfoCommitUrl(e){let t=e?.source?.commitUrl;return typeof t===\`string\`&&/^https:\\/\\/github\\.com\\/[^/\\s]+\\/[^/\\s]+\\/commit\\/[0-9a-f]{7,40}$/i.test(t)?t:null}function codexLinuxBuildInfoRevision(e){let t=e?.source??{},n=t.shortCommit||t.commit;return n?t.dirty?\`\${n} (dirty)\`:n:\`unknown\`}function codexLinuxBuildInfoDetail(e){if(!e)return\`No Linux build metadata file was found in this app install.\`;let t=e.linuxTarget??{},n=t.distro??{},r=e.upstreamDmg??{},i=e.source??{},a=e.linuxFeatures?.enabled??[],o=e.packageProfile??{},s=codexLinuxBuildInfoRevision(e),l=n.prettyName||[n.id,n.versionId].filter(Boolean).join(\` \`)||\`unknown\`,u=codexLinuxBuildInfoCommitUrl(e);return[\`Linux package profile: \${codexLinuxBuildInfoValue(o.label)}\`,\`Distro: \${l}\`,\`Package manager: \${codexLinuxBuildInfoValue(t.packageManager??o.packageManager)}\`,\`Package format: \${codexLinuxBuildInfoValue(t.packageFormat??o.format)}\`,\`Enabled features: \${a.length>0?a.join(\`, \`):\`none\`}\`,\`Upstream app version: \${codexLinuxBuildInfoValue(r.appVersion)}\`,\`Upstream DMG SHA256: \${codexLinuxBuildInfoValue(r.sha256)}\`,\`Electron: \${codexLinuxBuildInfoValue(e.electronVersion)}\`,\`Linux source revision: \${s}\`,...(u?[\`Source commit URL: \${u}\`]:[]),\`Source branch: \${codexLinuxBuildInfoValue(i.branch)}\`,\`Generated: \${codexLinuxBuildInfoValue(e.generatedAt)}\`].join(\`\\n\`)}function codexLinuxConfigureAboutPanel(){try{if(process.platform===\`linux\`){let e=codexLinuxReadBuildInfo(),t=e?.upstreamDmg??{},n=codexLinuxBuildInfoRevision(e),r=codexLinuxBuildInfoValue(t.appVersion),i=codexLinuxBuildInfoCommitUrl(e);${electronVar}.app?.setAboutPanelOptions?.({applicationName:\`Codex Desktop\`,applicationVersion:r,version:n,website:i??\`https://github.com/rabesss/codex-linux\`,copyright:\`Linux source revision: \${n}\`})}}catch{}}async function codexLinuxShowBuildInfo(){try{let e=codexLinuxReadBuildInfo(),t=codexLinuxBuildInfoCommitUrl(e),n=t?[\`Open Commit\`,\`OK\`]:[\`OK\`],r=await ${electronVar}.dialog?.showMessageBox({type:\`info\`,buttons:n,defaultId:t?1:0,cancelId:t?1:0,noLink:!0,message:\`Codex Desktop build information\`,detail:codexLinuxBuildInfoDetail(e)});t&&r?.response===0&&await ${electronVar}.shell?.openExternal(t)}catch{}}codexLinuxConfigureAboutPanel()`;
}
...
function applyLinuxBuildInfoTrayPatch(currentSource) {
  ...
  const aboutRoleRegex = /\{role:`about`([^}]*)\}/g;
  if (
    patchedSource.includes("role:`about`") &&
    !patchedSource.includes("codexLinuxShowBuildInfo(),codexLinuxConfigureAboutPanel()")
  ) {
    patchedSource = patchedSource.replace(aboutRoleRegex, (match, rest) => {
      if (match.includes("click:")) {
        return match;
      }
      changed = true;
      return `{role:\`about\`${rest},click:()=>{codexLinuxConfigureAboutPanel(),codexLinuxShowBuildInfo()}}`;
    });
  }
