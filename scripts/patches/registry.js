...
const OPT_IN = "opt-in";
const CONTROLLED_WORKSTATION = "controlled-workstation";
...
function allPatchPolicies(options = {}) {
  return [
    ...corePatchDescriptors(options).map(({ id, ciPolicy, phase, appliesTo, enabled }) => ({
      name: id,
      ciPolicy,
      phase,
      appliesTo,
    ...
    ...featurePatchDescriptors(options).map(({ id, ciPolicy, phase, appliesTo, enabled }) => ({
      name: id,
      ciPolicy,
      phase,
      appliesTo,
...
function requiredPatchNamesForProfile(profile, options = {}) {
  if (profile !== "upstream-build" && profile !== CONTROLLED_WORKSTATION) {
  ...
  return allPatchPolicies(options)
    .filter((patch) => profile === CONTROLLED_WORKSTATION || patch.ciPolicy === REQUIRED_UPSTREAM)
    .filter((patch) => patch.appliesTo == null || patch.appliesTo(context) !== false)
    .filter((patch) => patch.enabled == null || patch.enabled(context) !== false)
