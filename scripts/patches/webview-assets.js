...
function patchLinuxUiFontAlias(extractedDir) {
  ...
  if (!fs.existsSync(indexPath) || !fs.existsSync(assetsDir)) {
    console.warn("WARN: Could not find webview files - skipping Linux UI font alias patch");
    return { changed: false, reason: "webview files are missing" };
  }

  const indexSource = fs.readFileSync(indexPath, "utf8");
  if (indexSource.includes('id="codex-linux-geist-font-alias"')) {
    return { changed: false };
  }

  const assets = fs.readdirSync(assetsDir);
  const regular = assets.find((name) => /^OpenAISans-Regular-.*\.woff2$/u.test(name));
  const medium = assets.find((name) => /^OpenAISans-Medium-.*\.woff2$/u.test(name));
  if (regular == null || medium == null) {
    console.warn("WARN: Could not find bundled OpenAI Sans fonts - skipping Linux UI font alias patch");
    return { changed: false, reason: "bundled OpenAI Sans fonts are missing" };
  }
  if (!indexSource.includes("</head>")) {
    console.warn("WARN: Could not find webview head marker - skipping Linux UI font alias patch");
    return { changed: false, reason: "webview head marker is missing" };
  }

  const style = [
    '    <style id="codex-linux-geist-font-alias">',
    `      @font-face { font-family: "Geist"; src: url("./assets/${regular}") format("woff2"); font-style: normal; font-weight: 100 449; font-display: swap; }`,
    `      @font-face { font-family: "Geist"; src: url("./assets/${medium}") format("woff2"); font-style: normal; font-weight: 450 900; font-display: swap; }`,
    "    </style>",
  ].join("\n");
  fs.writeFileSync(indexPath, indexSource.replace("</head>", `${style}\n  </head>`), "utf8");
  return { changed: true };
}

function applyLinuxStatsigWorkspaceTypePatch(currentSource) {
  const legacyHelper = "function codexLinuxSanitizeStatsigUser(e){let t=e?.user?.custom?.workspace_type;return t!==null&&typeof t===`object`&&!Array.isArray(t)&&delete e.user.custom.workspace_type,e}";
  const helper = "function codexLinuxSanitizeStatsigUser(e){let t=e?.user?.custom?.workspace_type;return(t===null||typeof t===`object`&&!Array.isArray(t))&&delete e.user.custom.workspace_type,e}";
  if (currentSource.includes("codexLinuxSanitizeStatsigUser")) {
    return currentSource.replace(legacyHelper, helper);
  }
  if (!currentSource.includes("post-login Statsig bootstrap")) {
    return currentSource;
  }
  const schemaRegex =
    /var ([A-Za-z_$][\w$]*)=Ke\(\[Y\(\),W\(\),Be\(\),Ve\(Y\(\)\),de\(\)\]\),([A-Za-z_$][\w$]*)=xe\(/u;
  const schemaMatch = currentSource.match(schemaRegex);
  const markerIndex = currentSource.indexOf("post-login Statsig bootstrap");
  const nearbyStart = Math.max(0, markerIndex - 5_000);
  const nearbySource = currentSource.slice(nearbyStart, markerIndex + 1_000);
  const nearbyParseMatches = [...nearbySource.matchAll(/([A-Za-z_$][\w$]*)\.parse\(JSON\.parse\(([A-Za-z_$][\w$]*)\)\)/gu)];
  const nearbyParseMatch = nearbyParseMatches.at(-1) ?? null;
  const parserVar = schemaMatch?.[2] ?? nearbyParseMatch?.[1] ?? null;
  const parseIndex = nearbyParseMatch == null ? -1 : nearbyStart + nearbyParseMatch.index;
  const insertionIndex = schemaMatch?.index ?? currentSource.lastIndexOf("async function ", parseIndex);
  if (parserVar == null || insertionIndex < 0) {
    console.warn("WARN: Could not find Statsig user schema insertion point - skipping workspace type compatibility patch");
    return currentSource;
  }
  const parseRegex = new RegExp(
    `${parserVar.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\.parse\\(JSON\\.parse\\(([A-Za-z_$][\\w$]*)\\)\\)`,
    "u",
  );
  if (!parseRegex.test(currentSource)) {
    console.warn("WARN: Could not find Statsig bootstrap parse call - skipping workspace type compatibility patch");
    return currentSource;
  }

  const withHelper =
    currentSource.slice(0, insertionIndex) + helper + currentSource.slice(insertionIndex);
  return withHelper.replace(
    parseRegex,
    `${parserVar}.parse(codexLinuxSanitizeStatsigUser(JSON.parse($1)))`,
  );
}
...
function applyLinuxBrowserUseExternalAvailabilityPatch(currentSource) {
  ...
  const alreadyPatched = () =>
    /featureName:`browser_use_external`[\s\S]{0,900}?navigator\.userAgent\.includes\(`Linux`\)/.test(
      currentSource,
    );
...
function applyLocalEnvironmentActionModalDraftPatch(currentSource) {
  ...
  const markerIndex = currentSource.indexOf("settings.localEnvironments.actions.add.description");
  const componentMatches = [
    ...currentSource.slice(0, markerIndex).matchAll(/function ([A-Za-z_$][\w$]*)\(e\)\{/g),
  ];
  const componentMatch = componentMatches.at(-1);
  ...
  const functionStart = componentMatch.index;

  const recentActionsIndex = currentSource.indexOf("local-env-recent-actions-by-key", functionStart);
  const functionEnd = currentSource.lastIndexOf("var ", recentActionsIndex);
  if (recentActionsIndex === -1 || functionEnd <= functionStart) {
    console.warn(
  ...
  const componentHeader = patchedFunction.match(
    /^function [A-Za-z_$][\w$]*\(e\)\{let ([A-Za-z_$][\w$]*)=\(0,[A-Za-z_$][\w$]*\.c\)\(101\),\{action:([A-Za-z_$][\w$]*),configPath:[A-Za-z_$][\w$]*,environment:[A-Za-z_$][\w$]*,hostConfig:[A-Za-z_$][\w$]*,onOpenSettings:[A-Za-z_$][\w$]*,onRunAction:[A-Za-z_$][\w$]*,onSaved:[A-Za-z_$][\w$]*,onUpdate:([A-Za-z_$][\w$]*),workspaceRoot:([A-Za-z_$][\w$]*)\}=e,/,
  );
  const reactVar = currentSource.match(
    /\(0,([A-Za-z_$][\w$]*)\.(?:useState|useRef|useEffect)\)\(/,
  )?.[1] ?? "Q";
  if (componentHeader == null || reactVar == null) {
    console.warn(
      "WARN: Could not find local environment action modal state aliases — skipping action input patch",
    );
    return currentSource;
  }
  const [, cacheVar, actionVar, updateVar, workspaceRootVar] = componentHeader;
  const stateNeedle = `workspaceRoot:${workspaceRootVar}}=e,`;
  const statePatch =
    `workspaceRoot:${workspaceRootVar}}=e,[codexLinuxActionDraft,codexLinuxSetActionDraft]=(0,${reactVar}.useState)(()=>${actionVar}),codexLinuxUpdateActionDraft=e=>(codexLinuxSetActionDraft(t=>({...t,...e})),${updateVar}(e)),`;
  const requiredReplacements = [
    ...
    {
      needle: `{...${actionVar},command:I,name:P}`,
...
function applyBrowserAnnotationScreenshotPatch(currentSource) {
  ...
  if (patchedSource.includes(selectedMarkerInScreenshotPatch)) {
    ...
    if (patchedSource.includes(currentMarkersPatch)) {
      ...
      patchedSource.includes(latestCommentPreloadMarkersPatch) ||
      patchedSource.includes(electron42CommentPreloadMarkersPatch)
    ) {
      // Already patched.
...
function applyLinuxApiKeyLoginValidationPatch(currentSource) {
  const helperName = "codexLinuxValidateApiKeyLogin";
  const validatorPrefix =
    "(function codexLinuxValidateApiKeyLogin(e){let t=String(e??``).trim(),n=t.toLowerCase();if(!/^sk-(?:proj-|svcacct-)?[A-Za-z0-9_-]{20,}$/.test(t)||/(?:^sk-(?:test|fake|placeholder|example|dummy)|(?:test|fake|placeholder|example|dummy)(?:[-_]|$))/.test(n))throw Error(`Invalid API key. Paste a full OpenAI API key from platform.openai.com.`);return t})";
  let changed = false;
  const handlerPattern =
    /("login-with-api-key(?:-for-host)?"\s*:\s*[A-Za-z_$][\w$]*\(async\(([A-Za-z_$][\w$]*),\{apiKey:([A-Za-z_$][\w$]*)\}\)=>\{)await \2\.loginWithApiKey\(\3\)(\}\))/gu;
  const patchedSource = currentSource.replace(
    handlerPattern,
    (match, prefix, managerVar, apiKeyVar, suffix) => {
      changed = true;
      if (match.includes(helperName)) {
        return match;
      }
      return `${prefix}${apiKeyVar}=${validatorPrefix}(${apiKeyVar});await ${managerVar}.loginWithApiKey(${apiKeyVar})${suffix}`;
    },
  );
  ...
  if (currentSource.includes("login-with-api-key") && currentSource.includes("loginWithApiKey")) {
    console.warn(
