...
async fn apply_packaged(
    ...
    if let Some(missing) = missing_build_dependency() {
        let body = format!(
            "A newer Codex Desktop Control build is available, but '{missing}' is needed to rebuild it. Install the build tools or update the package manually."
        );
        let _ = notify::send("Codex Desktop Control update available", &body);
        println!("{body}");
